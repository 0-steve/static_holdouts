...
# get camapign code from misc to use in output file name
def campaign_title(mail_file_df): 
    campaign_code = list({ code.split("-")[0] for code in mail_file_df.misc })[0]
...
if __name__ == '__main__':
    ...
    print("Loading files...")
    print("")
    mail_file_df = pd.read_csv(args.mail_file, low_memory=False)
    misc_df = pd.read_csv("city_hold_market_id_2022-03-31.csv", low_memory=False)
    holdout_df = pd.read_csv("citiesHoldout_summary_2022-03-24.csv", low_memory=False)
    ...
    campaign_code = campaign_title(mail_file_df)
    current_date = date.today()

    holdout_checks(holdout_final_df, holdout_df) # if fail then stop

    os.mkdir(f"/private/tmp/{campaign_code}") 
    os.chdir(f"/private/tmp/{campaign_code}")

    final_name = f"{campaign_code}_{current_date}_StaticHoldouts.csv"
    holdout_final_df.to_csv(final_name, index=False)
  
    summary_name = f"{campaign_code}_{current_date}_StaticHoldouts_Summary.csv"
    holdout_summmary_df.to_csv(summary_name, index=False)
    print(f"Files out put in /private/tmp/{campaign_code}")
    print("")


# holdout_checks = return file if counts are off // if counts are off stop script